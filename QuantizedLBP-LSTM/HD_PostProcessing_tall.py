...
for Patient in range(10,16): # subj no.10-15
	...
	for j in range(0,len(predlen)):
		for k in range(0,Patient_Seizure[Patient-10]):
			...
			err1 = 0
			# read the result
			prediction = np.load('./result_hardware/'+"Patient"+str(Patient)+'_k'+str(k)+'.npy')
	...
	# 另存为mat矩阵后续分析
	scipy.io.savemat('result_hardlbplstm_'+str(Patient)+'_t5.mat', mdict={'spe': Acc[:,:,0], 'sen':  Acc[:,:,1], 'acc':  Acc[:,:,2]})
