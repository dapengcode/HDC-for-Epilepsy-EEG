...
for k in range(0,Patient_Seizure[Patient-10]):
	...
	x_train = torch.from_numpy(x_train*1e5).cuda()
	...
	for i in range(0,x_train.shape[0]):
		...
		if y_train[i] == seizure_type:
			if num_Seizure == 0:
				temp3=model.learn_HD_proj(x_train[i,:,0:250],slen) # 
				queeryVectorS1 = temp3 + 0
				num_Seizure = num_Seizure + 1
				temp3=model.learn_HD_proj(x_train[i,:,250:500],slen) # 
	...
	while i < x_train.shape[0]:
		if y_train[i] == 0:
			if num_noSeizure == 0:
				temp1=model.learn_HD_proj(x_train[i,:,0:250],slen) # 
				queeryVectorS0 = temp1 + 0
				num_noSeizure = num_noSeizure + 1
				temp1=model.learn_HD_proj(x_train[i,:,250:500],slen) # 
	...
	x_test = torch.from_numpy(x_test*1e5).cuda()

	for i in range(0,num_test):
		if i%2 == 0:
			temp = model.learn_HD_proj(x_test[i//2,:,0:250],slen) # 前0.5second
			[distanceVectorsS1[0,i],distanceVectorsS0[0,i],prediction0[0,i]] = model.predict(temp, queeryVectorS1, queeryVectorS0, D)
		else:
			temp = model.learn_HD_proj(x_test[i//2,:,250:500],slen) # 后0.5second
			[distanceVectorsS1[0,i],distanceVectorsS0[0,i],prediction0[0,i]] = model.predict(temp, queeryVectorS1, queeryVectorS0, D)
		if i%10000 == 0:
			print(i)
